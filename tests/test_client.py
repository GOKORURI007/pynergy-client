...
from src.client import get_screen_size, main, SynergyClient
...
class TestSynergyClientInit:
    ...
    def test_default_initialization(self):
        ...
        assert client._client_name == 'Pynergy'
    ...
    def test_custom_initialization(self):
        ...
        assert client._client_name == 'CustomClient'
...
class TestConnect:
    ...
    def test_connect(self):
        ...
        with patch('socket.socket') as mock_socket_class:
            ...
            hello_header = struct.pack('>I', len(hello_body))
...
class TestHandleMessage:
    ...
    def _make_active_client(self, device=None):
        """创建一个处于 ACTIVE 状态的客户端"""
        if device is None:
            device = MagicMock()
        client = SynergyClient('192.168.1.100', coords_mode='relative', device=device)
        client._state = ClientState.ACTIVE
        return client

    def test_handle_dmmv_relative(self):
        """测试相对坐标模式处理鼠标移动"""
        device = MagicMock()
        client = self._make_active_client(device)
        client._abs_to_rel(0, 0)
    ...
    def test_handle_cinn_transitions_to_active(self):
        """测试 CINN 消息切换到 ACTIVE 状态"""
        device = MagicMock()
        client = SynergyClient('192.168.1.100', device=device)
        assert client._state == ClientState.DISCONNECTED

        client._handle_message(MessageType.CINN, {'x': 0, 'y': 0, 'sequence': 1, 'modifiers': 0})

        assert client._state == ClientState.ACTIVE

    def test_handle_cout_transitions_to_connected(self):
        """测试 COUT 消息切换到 CONNECTED 状态"""
        device = MagicMock()
        client = self._make_active_client(device)

        client._handle_message(MessageType.COUT, {'sequence': 1})

        assert client._state == ClientState.CONNECTED

    def test_handle_cbye_stops_client(self):
        """测试 CBYE 消息停止客户端"""
        device = MagicMock()
        client = self._make_active_client(device)
        client._running = True

        client._handle_message(MessageType.CBYE, {})

        assert client._running is False

    def test_handle_qinf_sends_dinf(self):
        """测试 QINF 消息触发 DINF 响应"""
        device = MagicMock()
        client = SynergyClient(
            '192.168.1.100',
            screen_width=1920,
        ...
        client._handle_message(MessageType.QINF, {})

        client._sock.send.assert_called_once()

    def test_handle_calv_sends_response(self):
        """测试 CALV 消息响应"""
        device = MagicMock()
        client = self._make_active_client(device)
        client._sock = MagicMock()

        client._handle_message(MessageType.CALV, {})

        client._sock.send.assert_called_once()

    def test_ignore_message_when_not_active(self):
        """测试非 ACTIVE 状态忽略输入消息"""
        device = MagicMock()
        client = SynergyClient('192.168.1.100', device=device)
        client._state = ClientState.CONNECTED

        client._handle_message(MessageType.DMMV, {'x': 100, 'y': 100})

        device.write_mouse_move.assert_not_called()
...
class TestMain:
    ...
    def test_cli_help(self):
        ...
        assert 'Deskflow 服务器 IP 地址' in result.output
    ...
    def test_cli_with_client_name(self):
        """测试 CLI 客户端名称参数"""
        with patch.object(SynergyClient, 'run') as mock_run:
            runner = CliRunner()
            result = runner.invoke(main, ['--server', '192.168.1.100', '-n', 'CustomName'])
            mock_run.assert_called_once()
